...
output "ecs_task_iam_role_arn" {
  ...
  value = var.ecs_task_role == true ? var.ecs_task_role_name_create == true ? module.ecs_task_iam_role[0].iam_role_arn : data.aws_iam_role.ecs_task_iam_role[0].arn : null
}

output "ecs_task_iam_role_name" {
  description = "Name of the ECS task role"
  value = var.ecs_task_role == true ? var.ecs_task_role_name_create == true ? module.ecs_task_iam_role[0].iam_role_name : data.aws_iam_role.ecs_task_iam_role[0].name : null
}
...
output "alb_id" {
  description = "If an ALB was created this will contain the ID of the load balancer"
  value = var.alb_enabled == true ? module.task_alb[0].alb_id : null
}
